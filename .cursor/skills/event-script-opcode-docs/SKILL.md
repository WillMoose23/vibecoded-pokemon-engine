...
- Changes to **`tools/extract_map_script_ops.py`**, **`tools/event_script_ops_generated.py`**, **`tools/event_script_schema.py`**, or **`tools/event_script_opcode_docs.py`**.
...
- **`tools/validate_map_events.py`** when event JSON rules touch scripts or sprites.
...
3. **Regenerate:** Run `python3 tools/extract_map_script_ops.py` from the repo root. Fix any meta/C++ mismatch errors until exit code **0**.
...
8. **Validation:** If map event JSON shape changes, update **`tools/validate_map_events.py`** and run `python3 tools/validate_map_events.py`.

## Verification

- `python3 tools/extract_map_script_ops.py` exits **0**.
- `python3 tools/validate_map_events.py` exits **0** (after sprite/script-related validator edits).
