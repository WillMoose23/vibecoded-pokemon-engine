...
class MapEditor:
    ...
    def relayout(self) -> None:
        ...
        self.palette_sel_h = self.font.get_linesize() + 6
        self.gear_rect = pygame.Rect(self.map_viewport_rect.right - 36, self.map_viewport_rect.y + 2, 32, chip_h - 4)
    ...
    def refresh_map_file_list(self) -> None:
        ...
        self.map_files = sorted(
            p for p in MAPS_DIR.glob("*.json") if p.name != MAPS_INDEX_NAME
        )
    ...
    def open_map_interactive(self) -> None:
        ...
        ensure_maps_dir()
        stems = sorted(
            p.stem for p in MAPS_DIR.glob("*.json") if p.name != MAPS_INDEX_NAME
        )
        if not stems:
    ...
    def draw(self) -> None:
        ...
        if self.map_drag_start and self.map_paint_current and self.edit_mode == "paint":
        ...
        if self.map_drag_start and self.map_paint_current and self.edit_mode == "walk":
            x0, y0 = self.map_drag_start
            x1, y1 = self.map_paint_current
            ax0, ax1 = sorted((x0, x1))
            ay0, ay1 = sorted((y0, y1))
        ...
        line_b = (
            ...
            f"Save As: Shift+S  ·  Help: {self.key_primary('toggle_help')}  ·  "
            f"Settings: *  ·  Scroll: wheel  ·  "
            f"Palette: Ctrl+wheel zoom · Shift+wheel pan H"
        )
        yc = blit_wrapped_text(self.screen, self.font_small, line_b, pygame.Rect(inner.x, yc, inner.w, inner.bottom - yc), (165, 170, 185))
        yc += 10
        ...
        if rest.h > self.font_small.get_linesize():
            yc = blit_wrapped_text(
                self.screen,
                self.font_small,
                (
            ...
            yc += 4
        if self.show_quickstart and yc < inner.bottom:
    ...
    def _draw_open_map_overlay(self) -> None:
        ...
        pygame.draw.rect(self.screen, (120, 120, 140), box, 2)
        self.screen.blit(
            self.font.render("Open map  —  ↑↓ Enter ·  Esc close ·  wheel scroll", True, (240, 240, 245)),
            (box.x + 12, box.y + 10),
        )
        lh = self.font_small.get_linesize() + 4
        top_y = box.y + 44
        ...
        while running:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if mouse_down and self.map_canvas_rect.collidepoint(event.pos) and self.edit_mode in (
                        "paint",
                        "walk",
                    ):
                        ...
                        if self.fill_mode:
                            ...
                            if (x0, y0) == (x1, y1):
                                ...
                                if bw * bh <= 1:
                                    ...
                                    bo = (x0, y0)
                                    self._flood_fill_at(
                                        x0, y0, erase_paint, brush_origin=bo,
                                    )
                            else:
                    ...
                    if del_like:
                        ...
                        self.show_quickstart = not self.show_quickstart
                    elif (
